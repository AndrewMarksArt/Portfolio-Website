import React from "react";
import ContactMe from "../../components/ContactMe";
import { Link } from "react-router-dom";
import "./case_study5.css";

export default function Project5() {
    return (
        <div className="project5--main-container">
            <div>
                <img className="project5--larg-img" src="../img/case_studies/project5/bw_banner.png" />
            </div>

            <div className="project5-headline-container">
                <h1 className="project5--title">Battle Warriors Vikings of ETH: Design & Launch of a Generative NFT Collection and Gaming Ecosystem</h1>
                <h2 className="project5--sub-head">
                    Massa pede class viverra laoreet ad odio viverra condimentum dictum risus velit phasellus. Ac fusce et placerat sem pharetra, pulvinar praesent. Tempor 
                    sapien ante dapibus accumsan dignissim sollicitudin, varius nostra fringilla. In, quis rutrum fermentum placerat. Tincidunt sagittis turpis platea fermentum 
                    ornare netus sodales pulvinar. Platea scelerisque euismod egestas quis. Cras a Ac. Porta. Ad. Natoque sed sit ornare maecenas nascetur rhoncus ante dignissim 
                    ut.
                </h2>
                <h2 className="project5--sub-head">
                    I was brought on by the project founder to help redsign the player experience to help onboard new players, bring more consistency to the game,
                    and help design new levels for players to explore.
                </h2>
            </div>

            <div className="project5--phase1-container">
                <p className="project5--challenge">
                    The Challenge
                </p>
                <h1 className="project5--title">
                    Client Meeting -- needs, thoughts, basic requirements
                </h1>
                <h2 className="project5--body">
                    My Role, Methodology & Approach Taken. 
                </h2>
                <h2 className="project5--body">
                    Client requirements & needs:
                </h2>
                <ol className="project5--list">
                    <li className="project5--list-item">
                        Confusing buttons & layouts
                    </li>
                    <li className="project5--list-item">
                        Hard to navigate & easy to get stuck
                    </li>
                    <li className="project5--list-item">
                        Mobile layout issues & information hidden, users didn't know they could scroll or where to go
                    </li>
                </ol>
                <h2 className="project5--body">
                    Design notes & ideas:
                </h2>
                <ol className="project5--list">
                    <li className="project5--list-item">
                        How to get started, where to go, who can do what?
                    </li>
                    <li className="project5--list-item">
                        For players with lots of Shogun Warriors -- way to many transaction, very hard to see all of there warriros, what can be done, 
                        and manage their army.
                    </li>
                    <li className="project5--list-item">
                        Mechanics & gameplay a bit confusing, not sure how to best use their Shoguns.
                    </li>
                </ol>

            </div>

            <div className="project5--sketch-group-container">
                <div className="project5--sketch-group-imgs">
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/face_sketch1.png"/>
                    </div>
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/face_sketch2.png"/>
                    </div>
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/face_sketch3.png"/>
                    </div>
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/new_items_sketch.png"/>
                    </div>
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/early_archer.png"/>
                    </div>
                </div>
                <p className="project5--img-desc">
                    The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                    landing page visually worked on those small screens.
                </p>
            </div>

            <div className="project5--phase1-container">

                <h2 className="project5--body">
                    First impressions matter alot, lets show how we can fix the landing page -- colored CTA button, disable tutorial and add a coming soon, move the whitepaper
                    to where twitter and discord are. Lets implement a simple quick travel navigation menu, and lets add some nudges/descriptions to help players find their 
                    way around the map.
                </h2>

                <h2 className="project5--body">
                    First impressions matter alot, lets show how we can fix the landing page -- colored CTA button, disable tutorial and add a coming soon, move the whitepaper
                    to where twitter and discord are. Lets implement a simple quick travel navigation menu, and lets add some nudges/descriptions to help players find their 
                    way around the map.
                </h2>

                <div className="project5--test-gen-container">
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/test_generated_viking.png"/>
                        <p className="project5--img-desc">old landing page with broken buttons, whitepaper that takes them away from the game, & no link to comminity/socials.</p>
                    </div>
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/test_generation.png"/>
                        <p className="project5--img-desc">old landing page with broken buttons, whitepaper that takes them away from the game, & no link to comminity/socials.</p>
                    </div>
                </div>

                <h2 className="project5--body">
                    First impressions matter alot, lets show how we can fix the landing page -- colored CTA button, disable tutorial and add a coming soon, move the whitepaper
                    to where twitter and discord are. Lets implement a simple quick travel navigation menu, and lets add some nudges/descriptions to help players find their 
                    way around the map.
                </h2>

                <div className="project5--video-container">
                    <video src="../videos/example_viking.mp4" autoPlay loop muted className="char-build-video"/>
                </div>
            </div>

            <div className="project5--phase1-container">
                <h1 className="project5--title">
                    Phase 2 -- wireframes & initial mock ups
                </h1>
                <h2 className="project5--body">
                    First impressions matter alot, lets show how we can fix the landing page -- colored CTA button, disable tutorial and add a coming soon, move the whitepaper
                    to where twitter and discord are. Lets implement a simple quick travel navigation menu, and lets add some nudges/descriptions to help players find their 
                    way around the map.
                </h2>

                <h2 className="project5--body">
                    First impressions matter alot, lets show how we can fix the landing page -- colored CTA button, disable tutorial and add a coming soon, move the whitepaper
                    to where twitter and discord are. Lets implement a simple quick travel navigation menu, and lets add some nudges/descriptions to help players find their 
                    way around the map.
                </h2>
            
                <div className="project5--banner-group-container">
                    <div className="project5--banner-group-imgs">
                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/banner1.png"/>
                        </div>
                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/banner2.png"/>
                        </div>
                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/banner3.png"/>
                        </div>
                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/banner4.png"/>
                        </div>
                    </div>
                    <p className="project5--img-desc">
                        The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.
                    </p>
                </div>

                <h2 className="project5--body">
                    First impressions matter alot, lets show how we can fix the landing page -- colored CTA button, disable tutorial and add a coming soon, move the whitepaper
                    to where twitter and discord are. Lets implement a simple quick travel navigation menu.
                </h2>

                <h2 className="project5--body">
                    First impressions matter alot, lets show how we can fix the landing page -- colored CTA button, disable tutorial and add a coming soon, move the whitepaper
                    to where twitter and discord are. Lets implement a simple quick travel navigation menu.
                </h2>

                <div className="project5--items-container">
                    <div>
                        <img className="project5--item-banner" src="../img/case_studies/project5/armory_weapons.png"/>
                    </div>
                    <div className="project5--item-chests">
                        <img className="project5--item-chest" src="../img/case_studies/project5/sm_chest.png"/>
                        <img className="project5--item-chest" src="../img/case_studies/project5/lrg_chest.png"/>
                    </div>
                </div>
                
                <h2 className="project5--body">
                    The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                    landing page visually worked on those small screens.
                </h2>

                <h2 className="project5--body">
                    On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                    from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                    very important to the client.
                </h2>

                <div className="project5--mocks-container">
                    <div>
                        <img className="project5--img-mobile" src="../img/case_studies/project5/helmets.png"/>
                        <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.</p>
                    </div>
                    <div>
                        <img className="project5--img-mobile" src="../img/case_studies/project5/axes.png"/>
                        <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.</p>
                    </div>
                    <div>
                        <img className="project5--img-mobile" src="../img/case_studies/project5/hammers.png"/>
                        <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.</p>
                    </div>
                    <div>
                        <img className="project5--img-mobile" src="../img/case_studies/project5/swords.png"/>
                        <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.</p>
                    </div>
                </div>

                <h2 className="project5--body">
                    Navigation was the largest issues users had when playing so even though navigation on mobile is more straight forward
                    than on larger screens, I used the same menu to create a sense of consistency & help show the user all the different areas they can travel to.
                </h2>

                <h2 className="project5--body">
                    Navigation was the largest issues users had when playing so even though navigation on mobile is more straight forward
                    than on larger screens, I used the same menu to create a sense of consistency & help show the user all the different areas they can travel to.
                </h2>

                <div className="project5--examples-container">
                    <div>
                        <img className="project5--giveaway-loop" src="../img/case_studies/project5/bg1.png"/>
                    </div>
                    <div>
                        <img className="project5--freya-cloak" src="../img/case_studies/project5/bg2.png"/>
                    </div>
                    <div>
                        <img className="project5--giveaway-loop" src="../img/case_studies/project5/bg3.png"/>
                    </div>
                    <div>
                        <img className="project5--freya-cloak" src="../img/case_studies/project5/bg4.png"/>
                    </div>
                    <div>
                        <img className="project5--giveaway-loop" src="../img/case_studies/project5/bg5.png"/>
                    </div>
                    <div>
                        <img className="project5--freya-cloak" src="../img/case_studies/project5/bg6.png"/>
                    </div>
                    <div>
                        <img className="project5--giveaway-loop" src="../img/case_studies/project5/bg7.png"/>
                    </div>
                    <div>
                        <img className="project5--freya-cloak" src="../img/case_studies/project5/bg8.png"/>
                    </div>
                </div>

                <h2 className="project5--body">
                    Navigation was the largest issues users had when playing so even though navigation on mobile is more straight forward
                    than on larger screens, I used the same menu to create a sense of consistency & help show the user all the different areas they can travel to.
                </h2>

                <h2 className="project5--body">
                    Navigation was the largest issues users had when playing so even though navigation on mobile is more straight forward
                    than on larger screens, I used the same menu to create a sense of consistency & help show the user all the different areas they can travel to.
                </h2>

                <div className="project5--examples-container">
                    <div>
                        <img className="project5--giveaway-loop" src="../img/case_studies/project5/vik1.png"/>
                    </div>
                    <div>
                        <img className="project5--freya-cloak" src="../img/case_studies/project5/vik2.png"/>
                    </div>
                    <div>
                        <img className="project5--giveaway-loop" src="../img/case_studies/project5/vik3.png"/>
                    </div>
                    <div>
                        <img className="project5--freya-cloak" src="../img/case_studies/project5/vik4.png"/>
                    </div>
                    <div>
                        <img className="project5--giveaway-loop" src="../img/case_studies/project5/vik5.png"/>
                    </div>
                    <div>
                        <img className="project5--freya-cloak" src="../img/case_studies/project5/vik6.png"/>
                    </div>
                    <div>
                        <img className="project5--giveaway-loop" src="../img/case_studies/project5/vik7.png"/>
                    </div>
                    <div>
                        <img className="project5--freya-cloak" src="../img/case_studies/project5/vik8.png"/>
                    </div>
                </div>

                <h2 className="project5--body">
                    Navigation was the largest issues users had when playing so even though navigation on mobile is more straight forward
                    than on larger screens, I used the same menu to create a sense of consistency & help show the user all the different areas they can travel to.
                </h2>

                <h2 className="project5--body">
                    Navigation was the largest issues users had when playing so even though navigation on mobile is more straight forward
                    than on larger screens, I used the same menu to create a sense of consistency & help show the user all the different areas they can travel to.
                </h2>

                <h2 className="project5--body">
                    <a className="project5--link-to-button" href="https://opensea.io/collection/warriors-of-ether-viking" target="_blank">
                        Here is the offical collection on OpenSea.
                    </a>
                </h2>
                
                <br></br>

            </div>

            <div className="project5--phase2-container">
                <br></br>
                <p className="project5--challenge">
                    Balance & Concistency
                </p>
                <h1 className="project5--title">
                    Redesigning areas with a consistant design
                </h1>
                <h2 className="project5--body">
                    take a look at some of the older screens, all very different which can cause some confusion & frustrate the players. 
                </h2>
                <h2 className="project5--body">
                    Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                    nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                </h2>
                <h2 className="project5--body">
                    Massa pede class viverra laoreet ad odio viverra condimentum dictum risus velit phasellus. Ac fusce et placerat sem pharetra, pulvinar praesent. Tempor 
                    sapien ante dapibus accumsan dignissim sollicitudin, varius nostra fringilla. In, quis rutrum fermentum placerat. Tincidunt sagittis turpis platea fermentum 
                    ornare netus sodales pulvinar. Platea scelerisque euismod egestas quis. Cras a Ac. Porta. Ad. Natoque sed sit ornare maecenas nascetur rhoncus ante dignissim 
                    ut.
                </h2>

                <div className="project5--redesign-container">
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/site_wireframe.png"/>
                        <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.</p>
                    </div>
                

                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/dapp_wireframe.png"/>
                        <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                        from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                        very important to the client.</p>
                    </div>

                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/bw_site.png"/>
                        <p className="project5--img-desc">Navigation was the largest issues users had when playing so even though navigation on mobile is more straight forward
                        than on larger screens, I used the same menu to create a sense of consistency & help show the user all the different areas they can travel to.</p>
                    </div>

                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/bw_dapp_site.png"/>
                        <p className="project5--img-desc">Navigation was the largest issues users had when playing so even though navigation on mobile is more straight forward
                        than on larger screens, I used the same menu to create a sense of consistency & help show the user all the different areas they can travel to.</p>
                    </div>
                </div>

                <h2 className="project5--body">
                    Massa pede class viverra laoreet ad odio viverra condimentum dictum risus velit phasellus. Ac fusce et placerat sem pharetra, pulvinar praesent. Tempor 
                    sapien ante dapibus accumsan dignissim sollicitudin, varius nostra fringilla. In, quis rutrum fermentum placerat. Tincidunt sagittis turpis platea fermentum 
                    ornare netus sodales pulvinar. Platea scelerisque euismod egestas quis. Cras a Ac. Porta. Ad. Natoque sed sit ornare maecenas nascetur rhoncus ante dignissim 
                    ut.
                </h2>

                <div className="project5--redesign-container">
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/site_wireframe.png"/>
                        <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.</p>
                    </div>
                

                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/dapp_wireframe.png"/>
                        <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                        from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                        very important to the client.</p>
                    </div>
                </div>

                <div>
                    <h1 className="project5--title">
                        Key issues & comments from users.
                    </h1>
                    <h2 className="project5--body">
                        Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                        nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                    </h2>

                    <h2 className="project5--body">
                    Design notes & ideas:
                    </h2>
                    <ol className="project5--list">
                        <li className="project5--list-item">
                            How to get started, where to go, who can do what?
                        </li>
                        <li className="project5--list-item">
                            For players with lots of Shogun Warriors -- way to many transaction, very hard to see all of there warriros, what can be done, 
                            and manage their army.
                        </li>
                        <li className="project5--list-item">
                            Mechanics & gameplay a bit confusing, not sure how to best use their Shoguns.
                        </li>
                    </ol>

                    <h2 className="project5--body">
                        Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                        nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                    </h2>

                    <div className="project5--medium-links-container">
                        <div className="project5--img-container">
                            <a href="https://battlevikingnft.medium.com/battle-warriors-vikings-of-eth-553df4cae393" target="_blank">
                                <img className="project5--medium-img" src="../img/case_studies/project5/bw_banner.png"/>
                                <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                                landing page visually worked on those small screens.</p>
                            </a>
                        </div>

                        <div className="project5--img-container">
                            <a href="https://battlevikingnft.medium.com/looks-rare-or-does-it-fc49b56dcc6a" target="_blank">
                                <img className="project5--medium-img" src="../img/case_studies/project5/power_stones.png"/>
                                <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                                from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                                very important to the client.</p>
                            </a>
                        </div>

                        <div className="project5--img-container">
                        <a href="https://battlevikingnft.medium.com/whats-in-a-rank-281d0aaaa9ed" target="_blank">
                                <img className="project5--medium-img" src="../img/case_studies/project5/stats_card.png"/>
                                <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                                from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                                very important to the client.</p>
                            </a>
                        </div>
                    </div>

                    <h2 className="project5--body">
                        Massa pede class viverra laoreet ad odio viverra condimentum dictum risus velit phasellus. Ac fusce et placerat sem pharetra, pulvinar praesent. Tempor 
                        sapien ante dapibus accumsan dignissim sollicitudin, varius nostra fringilla. In, quis rutrum fermentum placerat. Tincidunt sagittis turpis platea fermentum 
                        ornare netus sodales pulvinar. Platea scelerisque euismod egestas quis. Cras a Ac. Porta. Ad. Natoque sed sit ornare maecenas nascetur rhoncus ante dignissim 
                        ut.
                    </h2>

                    <h2 className="project5--body">
                        Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                        nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                    </h2>

                    <div className="project5--medium-links-container">
                        <div className="project5--img-container">
                            <a href="https://battlevikingnft.medium.com/battle-warriors-vikings-of-eth-553df4cae393" target="_blank">
                                <img className="project5--medium-img" src="../img/case_studies/project5/bw_site.png"/>
                                <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                                landing page visually worked on those small screens.</p>
                            </a>
                        </div>

                        <div className="project5--img-container">
                            <a href="https://battlevikingnft.medium.com/looks-rare-or-does-it-fc49b56dcc6a" target="_blank">
                                <img className="project5--medium-img" src="../img/case_studies/project5/discord.png"/>
                                <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                                from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                                very important to the client.</p>
                            </a>
                        </div>

                        <div className="project5--img-container">
                        <a href="https://battlevikingnft.medium.com/whats-in-a-rank-281d0aaaa9ed" target="_blank">
                                <img className="project5--medium-img" src="../img/case_studies/project5/bw_twitter.png"/>
                                <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                                from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                                very important to the client.</p>
                            </a>
                        </div>
                    </div>

                    <h2 className="project5--body">
                        Massa pede class viverra laoreet ad odio viverra condimentum dictum risus velit phasellus. Ac fusce et placerat sem pharetra, pulvinar praesent. Tempor 
                        sapien ante dapibus accumsan dignissim sollicitudin, varius nostra fringilla. In, quis rutrum fermentum placerat. Tincidunt sagittis turpis platea fermentum 
                        ornare netus sodales pulvinar. Platea scelerisque euismod egestas quis. Cras a Ac. Porta. Ad. Natoque sed sit ornare maecenas nascetur rhoncus ante dignissim 
                        ut.
                    </h2>

                </div>
            </div>
            
            <div className="project5--phase1-container">
                <br></br>
                <br></br>
                <br></br>

                <h1 className="project5--title">
                    Art Gen & Code
                </h1>

                <h2 className="project5--body">
                    Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                    nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                </h2>

                <h2 className="project5--body">
                    Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                    nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                </h2>

                <h2 className="project5--body">
                    <a className="project5--link-to-button" href="https://colab.research.google.com/drive/1nEi4GZ3nvS39wP7S1XKtQcEkcbsCPH8U?usp=sharing" target="_blank">
                        You can see the Google Colab Notebook I used to test generating Vikings and traits here.
                    </a>
                </h2>
                <h2 className="project5--body">
                    <a className="project5--link-to-button" href="https://opensea.io/collection/warriors-of-ether-viking" target="_blank">
                        Here is the offical collection on OpenSea.
                    </a>
                </h2>

                <br></br>

                <div className="project5--traits-container">
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/set_traits_1.png"/>
                        <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.</p>
                    </div>
                

                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/set_traits_2.png"/>
                        <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                        from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                        very important to the client.</p>
                    </div>
                </div>

                <h2 className="project5--body">
                    Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                    nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                </h2>

                <div className="project5--wide-container">
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/df_head.png"/>
                        <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.</p>
                    </div>
                </div>

                <h2 className="project5--body">
                    Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                    nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                </h2>

                <h2 className="project5--body">
                    Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                    nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                </h2>

                <div className="project5--counts-container">
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/bg_counts.png"/>
                        <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.</p>
                    </div>
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/counts_2.png"/>
                        <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.</p>
                    </div>
                </div>

                <h2 className="project5--body">
                    Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                    nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                </h2>

                <h2 className="project5--body">
                    Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                    nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                </h2>

                <div className="project5--weapon-count-container">
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/weapon_count_1.png"/>
                        <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.</p>
                    </div>
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/weapon_count_2.png"/>
                        <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.</p>
                    </div>
                </div>

                <h2 className="project5--body">
                    Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                    nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                </h2>

                <h2 className="project5--body">
                    Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                    nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                </h2>

                <div className="project5--weapon-count-container">
                    <div className="project5--img-container">
                        <img className="project5--img" src="../img/case_studies/project5/viking_113.png"/>
                        <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                        landing page visually worked on those small screens.</p>
                    </div>
                </div>

                <h2 className="project5--body">
                    Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                    nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                </h2>

                <h2 className="project5--body">
                    Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                    nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                </h2>

                <h2 className="project5--body">
                    <a className="project5--link-to-button" href="https://colab.research.google.com/drive/1nEi4GZ3nvS39wP7S1XKtQcEkcbsCPH8U?usp=sharing" target="_blank">
                        You can see the Google Colab Notebook I used to test generating Vikings and traits here.
                    </a>
                </h2>
                <h2 className="project5--body">
                    <a className="project5--link-to-button" href="https://opensea.io/collection/warriors-of-ether-viking" target="_blank">
                        Here is the offical collection on OpenSea.
                    </a>
                </h2>
                <br></br>
            </div>

            {/*
            Phase 4 Game Design
            */}

            <div className="project5--phase2-container">
                    <br></br>
                    <p className="project5--challenge">
                        Survival Grounds side scrolling mini game.
                    </p>
                    <h1 className="project5--title">
                        Survival Grounds side scrolling mini game.
                    </h1>
                    <h2 className="project5--body">
                        take a look at some of the older screens, all very different which can cause some confusion & frustrate the players. 
                    </h2>
                    <h2 className="project5--body">
                        Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                        nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                    </h2>
                    <h2 className="project5--body">
                        Massa pede class viverra laoreet ad odio viverra condimentum dictum risus velit phasellus. Ac fusce et placerat sem pharetra, pulvinar praesent. Tempor 
                        sapien ante dapibus accumsan dignissim sollicitudin, varius nostra fringilla. In, quis rutrum fermentum placerat. Tincidunt sagittis turpis platea fermentum 
                        ornare netus sodales pulvinar. Platea scelerisque euismod egestas quis. Cras a Ac. Porta. Ad. Natoque sed sit ornare maecenas nascetur rhoncus ante dignissim 
                        ut.
                    </h2>

                    <div className="project5--game-container">
                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/game_home.png"/>
                            <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                            landing page visually worked on those small screens.</p>
                        </div>
                    </div>

                    <h2 className="project5--body">
                        Massa pede class viverra laoreet ad odio viverra condimentum dictum risus velit phasellus. Ac fusce et placerat sem pharetra, pulvinar praesent. Tempor 
                        sapien ante dapibus accumsan dignissim sollicitudin, varius nostra fringilla. In, quis rutrum fermentum placerat. Tincidunt sagittis turpis platea fermentum 
                        ornare netus sodales pulvinar. Platea scelerisque euismod egestas quis. Cras a Ac. Porta. Ad. Natoque sed sit ornare maecenas nascetur rhoncus ante dignissim 
                        ut.
                    </h2>

                    <div className="project5--ex-levels-container">
                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/ex_spikes.png"/>
                            <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                            landing page visually worked on those small screens.</p>
                        </div>

                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/ex_arrow.png"/>
                            <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                            from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                            very important to the client.</p>
                        </div>

                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/ex_arrow_axes.png"/>
                            <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                            from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                            very important to the client.</p>
                        </div>
                    </div>

                    <h2 className="project5--body">
                        Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                        nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                    </h2>

                    <h2 className="project5--body">
                        Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                        nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                    </h2>

                    <div className="project5--char-selects-container">
                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/char_select.png"/>
                            <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                            landing page visually worked on those small screens.</p>
                        </div>

                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/char_confirm.png"/>
                            <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                            from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                            very important to the client.</p>
                        </div>
                    </div>

                    <h2 className="project5--body">
                        Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                        nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                    </h2>

                    <h2 className="project5--body">
                        Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                        nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                    </h2>

                    <div className="project5--ex-levels-container">
                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/help1.png"/>
                            <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                            landing page visually worked on those small screens.</p>
                        </div>

                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/help2.png"/>
                            <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                            from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                            very important to the client.</p>
                        </div>

                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/help3.png"/>
                            <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                            from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                            very important to the client.</p>
                        </div>
                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/help4.png"/>
                            <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                            landing page visually worked on those small screens.</p>
                        </div>

                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/help5.png"/>
                            <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                            from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                            very important to the client.</p>
                        </div>

                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/help6.png"/>
                            <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                            from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                            very important to the client.</p>
                        </div>
                    </div>

                    <div>
                        <h1 className="project5--title">
                            Key issues & comments from users.
                        </h1>
                        <h2 className="project5--body">
                            Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                            nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                        </h2>

                        <h2 className="project5--body">
                        Design notes & ideas:
                        </h2>
                        <ol className="project5--list">
                            <li className="project5--list-item">
                                How to get started, where to go, who can do what?
                            </li>
                            <li className="project5--list-item">
                                For players with lots of Shogun Warriors -- way to many transaction, very hard to see all of there warriros, what can be done, 
                                and manage their army.
                            </li>
                            <li className="project5--list-item">
                                Mechanics & gameplay a bit confusing, not sure how to best use their Shoguns.
                            </li>
                        </ol>

                        <h2 className="project5--body">
                            Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                            nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                        </h2>

                        <div className="project5--ex-levels-container">
                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/free_chars.png"/>
                            <p className="project5--img-desc">The game didn't work well on mobile but many players wanted to use mobile or smaller tablets so I made sure the new 
                            landing page visually worked on those small screens.</p>
                        </div>

                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/free_char_select.png"/>
                            <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                            from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                            very important to the client.</p>
                        </div>

                        <div className="project5--img-container">
                            <img className="project5--img" src="../img/case_studies/project5/free_char_confirm.png"/>
                            <p className="project5--img-desc">On smaller screens & in portrait mode the original world map layout didn't work, so I created cards that showed the locations
                            from the map overlayed on the world map. This way players on mobile can navigate easily where they need to go & the artwork is still highlighted which was 
                            very important to the client.</p>
                        </div>
                    </div>

                        <h2 className="project5--body">
                            Massa pede class viverra laoreet ad odio viverra condimentum dictum risus velit phasellus. Ac fusce et placerat sem pharetra, pulvinar praesent. Tempor 
                            sapien ante dapibus accumsan dignissim sollicitudin, varius nostra fringilla. In, quis rutrum fermentum placerat. Tincidunt sagittis turpis platea fermentum 
                            ornare netus sodales pulvinar. Platea scelerisque euismod egestas quis. Cras a Ac. Porta. Ad. Natoque sed sit ornare maecenas nascetur rhoncus ante dignissim 
                            ut.
                        </h2>

                        <h2 className="project5--body">
                            Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                            nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                        </h2>

                        <div className="project5--video-container">
                            <video src="../videos/free_play_prototype.mp4" autoPlay loop muted className="free-play-video"/>
                        </div>

                        <h2 className="project5--body">
                            Massa pede class viverra laoreet ad odio viverra condimentum dictum risus velit phasellus. Ac fusce et placerat sem pharetra, pulvinar praesent. Tempor 
                            sapien ante dapibus accumsan dignissim sollicitudin, varius nostra fringilla. In, quis rutrum fermentum placerat. Tincidunt sagittis turpis platea fermentum 
                            ornare netus sodales pulvinar. Platea scelerisque euismod egestas quis. Cras a Ac. Porta. Ad. Natoque sed sit ornare maecenas nascetur rhoncus ante dignissim 
                            ut.
                        </h2>

                        <h2 className="project5--body">
                            Fringilla nullam aenean rutrum morbi nunc hac purus mattis congue, vel pellentesque scelerisque donec, egestas dapibus. Ultricies elit Feugiat id. Habitasse 
                            nisi sodales ac mollis ante volutpat dictumst sagittis feugiat dictum fermentum cras, suspendisse elit.
                        </h2>

                        <div className="projec4--footer"> 
                            <br></br> 
                            <Link to="/projects" style={{ textDecoration: 'underline' }} className="project5--back-to-button">Back to projects </Link>
                            <br></br>
                        </div>

                    </div>
                </div>
        </div>
    )
}
